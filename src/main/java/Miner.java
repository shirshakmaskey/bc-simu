...
//      System.out.println("Running miner thread: " + this.getName());
        ...
        while (!consensusAchieved()) {
            while (!solutionClaimed && !Blockchain.numLeading0is(difficulty, Encryption.sha256("" + nounce + prevInfo))) {
                nounce++;
                if (nounce == Integer.MAX_VALUE
                        && !Blockchain.numLeading0is(difficulty, Encryption.sha256("" + nounce + prevInfo))) {
                    prevInfo++;
            ...
            if (solutionClaimed) {
                ...
                if (Blockchain.numLeading0is(difficulty, Encryption.sha256("" + candidate + prevInfo))) {
                    consensusList.set(index, true);
                } else {
                    // if this candidate fails the verification
                    resetConsensus();
                }
            } else if (Blockchain.numLeading0is(difficulty, Encryption.sha256("" + nounce + prevInfo))) {
